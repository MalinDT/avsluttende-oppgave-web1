...
Under er en funksjon filterArray som bruker .filter metoden på odds og evens const'ene. 
Du skal fikse getOdds og getEvens arrow funksjonene for å hente ut de riktige dataene til de forskjellige const'ene.  
*/
...
Under skal du lage en if else logikk som skal endre result og valueDifference til en ny verdi. Litt av logikken er med allerede som sjekker om sumOfOdds er større enn sumOfEvens. 
Bruk denne tankegangen videre for å lage tilsvarende logikk hvis sumOfOdds er mindre enn sumOfEvens:
...
if (sumOfOdds > sumOfEvens) {
  ...
  result = "Sum of Even is greater than sum of Odds"
} else {
  valueDifference = 0
  result = "sumOfEvens og sumOfOdds er like"
}
...
Lag en ny conditional logikk under hvor man sjekker om man har flere oddetall enn partall eller motsatt 
(syntax messig er det veldig likt som i oppgave 4, men man sjekker andre ting, ikke sumOfOdds og sumOfEvens). 
typeDifference skal endres til en template literal string med hensiktsmessig tekst for å forklare hva som skjer.
...
- Finn gjennomsnittsverdien av oddetallene og partallene (burde være 2 nye variabler som returnerer 
gjennomsnittsverdien. Disse brukes i if/else du skal skrive som conditionals, 
men også for å vise fram riktig verdi i template literalen du skal lage)
- Lag en ny variabel for referanse til verdien du skal vise fram (tilsvarende slik typeDifference og 
valueDifference ble laget i de forrige oppgavene)
- Lag en ny conditional tilsvarende som i oppgave 4 og 5 for å få ut en template literal som vises på nettsiden.
- Lag en ny p tag i html filen med en id for referanse og skriv inn en ny document. getElementById for å få 
fanget den opp i javascript filen her:
...
let Odd = sumOfOdds / odds.length

let Even = sumOfEvens / evens.length

if (Odd > Even) {
  variabel = `Gjennomsnitt til odd er: ${Odd}`
} else if (Even > Odd) {
  variabel = `Gjennomsnitt til Even er: ${Even}`
} else {
  variabel = "Variablene er like store"
}
...
f.eks median, størst verdi, minst verdi, hvor mange verdier er over/under en viss verdi, er noen av verdiene enn spesifikk verdi, 
er noen av verdiene fra odds og evens like.
...
document.getElementById("Oppgave").textContent = variabel;
